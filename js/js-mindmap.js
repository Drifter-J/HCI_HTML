...
var click = function (){
  ...
  function renderProductList(element, index, arr) {
    var node_prime = node.cloneNode(true);
    node_prime.addEventListener
...
(function($){   
    ...
    function Node(obj, name, parent, opts){
        ...
        image.onload = function () {
            ctx.drawImage(image, 0, 0, 50, 50); // draw the image on the canvas
        }
        ...
        $('#picker').mousemove(function(e) { // mouse move handler
            if (bCanPreview) {
                ...
                // update color
                document.getElementsByClassName("node ui-draggable active")[0].style.backgroundColor = "rgb("+pixel[0]+", "+pixel[1]+", "+pixel[2]+")";
    ...
    Node.prototype.updatePosition = function(){
      ...
      this.el.css('left', (showx/1.5)+150 + "px");
      this.el.css('top', (showy/1.5)+50 + "px");
      return false;
    }
...
//                    this.obj.ctx.lineWidth = "1";
        ...
        //display edge
        var c = this.obj.canvas.path("M"+(this.start.x/1.5)+' '
            +this.start.y/1.5+"L"+this.end.x/1.5+' '
            +this.end.y/1.5).attr({stroke: this.strokeStyle, opacity:0.2, 'stroke-width':'5px'});      
    }
    ...
    $.fn.mindmap = function(options) {
        ...
        return this.each(function() {
            ...
            //create drawing area
            this.canvas = Raphael(170, 70, options.mapArea.x-170, options.mapArea.y-100);
